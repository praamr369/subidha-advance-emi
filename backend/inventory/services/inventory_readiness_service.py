...
def get_inventory_readiness_snapshot() -> dict[str, Any]:
    ...
    if stock_needs_open:
        warnings.append(
            {
                "code": "OPEN_STOCK_NEEDS",
                "message": (
                    f"{stock_needs_open} operational purchase/stock need row(s) are still open. "
                    "Global ATP readiness (inventory_ready) can still be true when tracked SKUs have on-hand stock; "
                    "review Stock Needs for shortages that block specific sales or deliveries."
                ),
            }
        )
        recommended_actions.append(
            "Review Stock Needs workspace, use Recheck stock when ATP changed, or close or fulfil outstanding requests."
        )
