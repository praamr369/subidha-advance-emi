...
def get_setup_readiness() -> dict[str, Any]:
    ...
    sections = [
        ...
        _section(key="staff_advance_workflow", title="Staff Advance Workflow", status="READY" if _active_system_account_exists("STAFF_ADVANCE_ASSET") else "REQUIRED_PENDING", warnings=[] if _active_system_account_exists("STAFF_ADVANCE_ASSET") else ["Staff Advance Receivable account is not configured."], recommended_action="Run Accounting Setup defaults, then create, approve, disburse, and recover advances through the HR Staff Advances workspace.", target_route="/admin/hr/staff-advances", why_this_matters="Staff advances require a real employee source, explicit approval, settlement account, journals, and recovery history.", category=OPTIONAL_OR_FUTURE, optional_for_initial_start=True, metadata={"posting_supported": True}),
    ]
    core_sections = [
        section
