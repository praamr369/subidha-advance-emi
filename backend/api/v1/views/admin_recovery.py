...
# ---------------------------------------------------------------------------
# Recovery automation — legal notice, settlement offer, bulk escalation
# ---------------------------------------------------------------------------

_LEGAL_NOTICE_SUBJECT = "[SUBIDHA] Legal Notice — Outstanding EMI Dues"
_LEGAL_NOTICE_BODY = """Dear {customer_name},

This is a formal legal notice regarding your outstanding dues under contract {contract_ref}.

Outstanding amount: ₹{overdue_amount}
Overdue EMIs: {overdue_emis}
First overdue date: {first_overdue_date}

You are required to clear the outstanding balance within 7 days of receipt of this notice, failing which we shall be constrained to initiate appropriate legal proceedings under applicable law.

Please contact us immediately to settle the dues or discuss a payment arrangement.

This notice has been generated by {company_name} on {today}.
...
_SETTLEMENT_OFFER_SUBJECT = "[SUBIDHA] Settlement Offer — Outstanding EMI Dues"
_SETTLEMENT_OFFER_BODY = """Dear {customer_name},

With reference to your account under contract {contract_ref}, we are pleased to offer you an opportunity to settle your outstanding dues.

Outstanding amount: ₹{overdue_amount}

We can consider a one-time settlement at a mutually agreed amount. Please contact us within 14 days to discuss the settlement terms.

This is a goodwill offer and does not waive any legal rights.
...
class AdminRecoveryCaseSendLegalNoticeView(APIView):
    """Send a legal notice email and advance stage to NOTICE_SENT."""
    permission_classes = [IsAuthenticated, IsAdmin]
    ...
    def post(self, request, pk):
        ...
        try:
            rc = RecoveryCase.objects.select_related(
                "subscription__customer", "subscription__product"
            ).get(pk=pk)
        except RecoveryCase.DoesNotExist:
            return Response({"detail": "Recovery case not found."}, status=404)

        if rc.stage in ("SETTLED", "WRITTEN_OFF"):
            return Response({"detail": f"Cannot send legal notice for a {rc.stage} case."}, status=400)

        customer = rc.subscription.customer
        email = (request.data.get("email") or customer.user.email or "").strip()
        if not email:
            return Response({"detail": "No email address. Pass 'email' in request body."}, status=400)

        try:
            company_name = getattr(django_settings, "COMPANY_NAME", "SUBIDHA")
        except Exception:
            company_name = "SUBIDHA"

        body = _LEGAL_NOTICE_BODY.format(
            customer_name=customer.name,
            contract_ref=rc.subscription.subscription_number or f"SUB-{rc.subscription_id}",
            overdue_amount=str(_money(rc.overdue_amount)),
            overdue_emis=rc.overdue_emis,
            first_overdue_date=str(rc.first_overdue_date) if rc.first_overdue_date else "N/A",
            company_name=company_name,
            today=str(timezone.localdate()),
        )
        try:
            send_mail(
                subject=_LEGAL_NOTICE_SUBJECT,
                message=body,
            ...
            return Response({"detail": f"Email dispatch failed: {exc}"}, status=500)

        # Advance stage and record timestamp
        now = timezone.now()
        if rc.stage == "IDENTIFIED":
            rc.stage = "NOTICE_SENT"
        if not rc.notice_sent_at:
            rc.notice_sent_at = now
        rc.last_contact_at = now
        rc.save(update_fields=["stage", "notice_sent_at", "last_contact_at", "updated_at"])
        ...
        return Response({
            ...
            "case_id": rc.id,
            "new_stage": rc.stage,
            "notice_sent_at": rc.notice_sent_at.isoformat(),
        })


class AdminRecoveryCaseSendSettlementOfferView(APIView):
    """Send a settlement offer email."""
    permission_classes = [IsAuthenticated, IsAdmin]
    ...
    def post(self, request, pk):
        ...
        if rc.stage in ("SETTLED", "WRITTEN_OFF"):
            return Response({"detail": f"Case is already {rc.stage}."}, status=400)
        ...
        company_name = getattr(django_settings, "COMPANY_NAME", "SUBIDHA")
        body = _SETTLEMENT_OFFER_BODY.format(
            customer_name=customer.name,
            contract_ref=rc.subscription.subscription_number or f"SUB-{rc.subscription_id}",
            overdue_amount=str(_money(rc.overdue_amount)),
            company_name=company_name,
        ...
        try:
            send_mail(
                subject=_SETTLEMENT_OFFER_SUBJECT,
                message=body,
                from_email=getattr(django_settings, "DEFAULT_FROM_EMAIL", None),
                recipient_list=[email],
                fail_silently=False,
        ...
        now = timezone.now()
        rc.last_contact_at = now
        rc.save(update_fields=["last_contact_at", "updated_at"])
...
class AdminRecoveryCaseBulkEscalateView(APIView):
    """
    Auto-escalate recovery cases based on aging:
      IDENTIFIED → NOTICE_SENT   if aging_days >= 30
      NOTICE_SENT → FIELD_VISIT  if aging_days >= 60
      FIELD_VISIT → LEGAL        if aging_days >= 90
    """
    permission_classes = [IsAuthenticated, IsAdmin]
    ...
    def post(self, request):
        ...
        today = timezone.localdate()
        qs = RecoveryCase.objects.exclude(stage__in=["SETTLED", "WRITTEN_OFF"])
        escalated = []
        now = timezone.now()

        for rc in qs.select_related("subscription__customer"):
            if not rc.first_overdue_date:
                continue
            days = (today - rc.first_overdue_date).days
            new_stage = None
            update_fields = []

            if rc.stage == "IDENTIFIED" and days >= 30:
                new_stage = "NOTICE_SENT"
                if not dry_run:
                    rc.stage = new_stage
                    if not rc.notice_sent_at:
                        rc.notice_sent_at = now
                        update_fields.append("notice_sent_at")
            elif rc.stage == "NOTICE_SENT" and days >= 60:
                new_stage = "FIELD_VISIT"
                if not dry_run:
                    rc.stage = new_stage
                    if not rc.field_visit_at:
                        rc.field_visit_at = now
                        update_fields.append("field_visit_at")
            elif rc.stage == "FIELD_VISIT" and days >= 90:
                new_stage = "LEGAL"
                if not dry_run:
                    rc.stage = new_stage
                    if not rc.legal_at:
                        rc.legal_at = now
                        update_fields.append("legal_at")

            if new_stage:
                escalated.append({
                    "case_id": rc.id,
                    "customer": rc.subscription.customer.name,
                    "from_stage": rc.stage if dry_run else None,
                    "to_stage": new_stage,
                    "aging_days": days,
                })
                if not dry_run:
                    update_fields += ["stage", "updated_at"]
                    rc.save(update_fields=list(set(update_fields)))
