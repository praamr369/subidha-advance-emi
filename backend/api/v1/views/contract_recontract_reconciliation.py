...
class AdminContractAmendmentProductRecontractReconciliationBridgeView(APIView):
    ...
    def post(self, request, pk: int):
        ...
        if not event:
            return Response({"detail": "No active product recontract event exists for this amendment."}, status=status.HTTP_400_BAD_REQUEST)

        try:
            payload = execute_product_recontract_reconciliation(event=event, requested_by=request.user)
