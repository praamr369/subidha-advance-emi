...
class ContractRecontractReconciliationBridgeTests(TestCase):
    ...
    def test_variance_blocks_when_balanced_amount_does_not_match_expected(self):
        ...
        self._prime(amendment)
        bridge = AccountingBridgePosting.objects.get(source_model="ContractRecontractEvent", purpose="CONTRACT_RECONTRACT_ACCOUNTING_ADJUSTMENT")
        JournalEntryLine.objects.filter(journal_entry=bridge.journal_entry).update(debit_amount=Decimal("0.00"), credit_amount=Decimal("0.00"))
        debit_line = bridge.journal_entry.lines.order_by("id").first()
        credit_line = bridge.journal_entry.lines.order_by("id").last()
        JournalEntryLine.objects.filter(pk=debit_line.pk).update(debit_amount=Decimal("4999.00"), credit_amount=Decimal("0.00"))
        JournalEntryLine.objects.filter(pk=credit_line.pk).update(debit_amount=Decimal("0.00"), credit_amount=Decimal("4999.00"))
        before = self._reconciliation_counts()
