...
from accounting.services import accounting_bridge_customer_advance_guard_service as previous
...
EVENT_KEY = "customer_advance_receipt"
PURPOSE = "CUSTOMER_ADVANCE_RECEIPT"
SKIPPED_EVENT_KEY = "customer_advance_receipt_skipped_not_applicable"
UNSUPPORTED_EVENT_KEY = "unsupported_customer_advance_receipt"
SAFETY_TEXT = (
    "Preview is read-only. Posting creates accounting entries only after explicit admin confirmation. "
    "It does not edit customer advance, allocation, payment, receipt, customer, contract, or finance-account records."
)
DUPLICATE_F2_REASON = "ReceiptDocument.customer_advance is handled by Phase F2 and must not be duplicated by F20."
F21_REASON = "Customer advance application rows are handled by F21, not F20 receipt posting."


def _ref(row: CustomerAdvance) -> str:
    return row.reference_no or f"CA-{row.id}"


def _metadata(row: CustomerAdvance) -> dict[str, Any]:
    return row.allocation_metadata if isinstance(row.allocation_metadata, dict) else {}


def _classify(row: CustomerAdvance) -> tuple[str, str, str | None]:
    metadata = _metadata(row)
    owner = str(metadata.get("source_model") or metadata.get("source_contract_owner") or "").strip()
    if owner == "ReceiptDocument" or metadata.get("receipt_document_id"):
        return SKIPPED_EVENT_KEY, "Customer advance receipt skipped", DUPLICATE_F2_REASON
    if metadata.get("collection_mode") == "ADVANCE_ALLOCATION":
        return SKIPPED_EVENT_KEY, "Customer advance receipt skipped", F21_REASON
    if base._money(row.amount) <= Decimal("0.00"):
        return UNSUPPORTED_EVENT_KEY, "Unsupported customer advance receipt", "CustomerAdvance amount must be greater than zero."
    if metadata.get("future_bridge_phase") not in {"F20_CUSTOMER_ADVANCE_RECEIPT", "CUSTOMER_ADVANCE_RECEIPT"}:
        return UNSUPPORTED_EVENT_KEY, "Unsupported customer advance receipt", "CustomerAdvance source metadata does not mark this row as F20 receipt evidence."
    if metadata.get("accounting_bridge_posting_deferred") is not True:
        return UNSUPPORTED_EVENT_KEY, "Unsupported customer advance receipt", "CustomerAdvance source metadata does not defer accounting bridge posting to F20."
    return EVENT_KEY, "Customer advance receipt", None


def _finance_account_blocker(row: CustomerAdvance) -> str | None:
    finance_account = row.finance_account
    if finance_account is None:
        return "Finance account is missing for this customer advance receipt."
    if not finance_account.is_active:
        return "Finance account is inactive for this customer advance receipt."
    if not finance_account.chart_account_id or not finance_account.chart_account.is_active:
        return "Finance account is not mapped to an active chart account for this customer advance receipt."
    return None


def _lines(row: CustomerAdvance, event_key: str):
    warnings: list[str] = []
    if event_key != EVENT_KEY:
        return [], ["Unsupported customer advance receipt event."], row.finance_account
    amount = base._money(row.amount)
    if amount <= Decimal("0.00"):
        warnings.append("CustomerAdvance amount must be greater than zero.")
    if not row.customer_id:
        warnings.append("CustomerAdvance customer evidence is missing.")
    if not row.payment_date:
        warnings.append("CustomerAdvance payment_date is required.")
    if not row.method:
        warnings.append("CustomerAdvance payment method is required.")
    finance_account = row.finance_account
    ...
    reference = _ref(row)
    return [
        {"chart_account": finance_account.chart_account, "description": f"Customer advance receipt {reference}", "debit_amount": amount, "credit_amount": Decimal("0.00")},
        {"chart_account": liability, "description": f"Customer advance liability {reference}", "debit_amount": Decimal("0.00"), "credit_amount": amount},
...
def _source_snapshot(row: CustomerAdvance) -> dict[str, Any]:
    return {
        ...
        "cash_counter_id": row.cash_counter_id,
        "amount": row.amount,
        ...
        "status": row.status,
        "notes": row.notes,
        "allocation_metadata": row.allocation_metadata,
        "collected_by_id": row.collected_by_id,
    }


def _linked_snapshot(row: CustomerAdvance) -> dict[str, Any]:
    return {
...
def _qs():
    return CustomerAdvance.objects.select_related(
        "customer",
        "finance_account",
...
def candidate_for(row: CustomerAdvance) -> dict[str, Any]:
    ...
    payload = base._candidate_payload(
        ...
        source_pk=row.id,
        source_display=f"Customer advance {_ref(row)}",
        source_reference=_ref(row),
        source_date=row.payment_date,
        amount=row.amount,
        lines=lines,
        ...
        idempotency_key=f"bridge:{PURPOSE}:CustomerAdvance:{row.id}:{source_date_key}:{base._money(row.amount):.2f}:{source_idempotency_key}",
    ...
    payload.update(
        {
            "customer_advance_id": row.id,
            "advance_reference": _ref(row),
            "reference_no": row.reference_no,
            "customer_id": row.customer_id,
            "customer_name": getattr(row.customer, "name", None),
            "amount": f"{base._money(row.amount):.2f}",
            "unapplied_amount": f"{base._money(row.unapplied_amount):.2f}",
            ...
            "advance_status": row.status,
            "source_metadata": metadata,
            "source_idempotency_key": metadata.get("source_idempotency_key"),
...
def get_bridge_candidate(candidate_id: str, *, for_update: bool = False) -> dict[str, Any]:
    ...
    if source_kind != "customeradvance":
    ...
    if candidate["event_key"] != event_key:
        raise ValueError("CustomerAdvance candidate event no longer matches current source state.")
    return candidate
...
def preview_bridge_candidate(candidate_id: str) -> dict[str, Any]:
    ...
    source = {
        ...
        "unapplied_amount": candidate.get("unapplied_amount"),
        ...
        "advance_status": candidate.get("advance_status"),
...
def post_bridge_candidate(*, candidate_id: str, idempotency_key: str, confirmed: bool, posting_note: str = "", actor) -> dict[str, Any]:
    ...
    if candidate["event_key"] != EVENT_KEY:
        raise ValueError("Unsupported CustomerAdvance bridge candidate source.")
    ...
    journal, created = post_bridge_entry(
        ...
        memo=f"Bridge posting CustomerAdvance {row.id} {EVENT_KEY}",
        lines=lines,
    ...
    if _source_snapshot(row) != source_before or _linked_snapshot(row) != linked_before:
        raise ValueError("CustomerAdvance source mutation detected; bridge posting rolled back.")
...
def _is_own(candidate_id: str) -> bool:
    try:
        ...
        return False
    return source_kind == "customeradvance"
...
def summarize_candidate_statuses(rows: list[dict[str, Any]]) -> dict[str, int]:
    ...
    summary.update(
        {
            "customer_advance_receipt_ready_unposted_count": sum(1 for row in rows if row.get("source_model") == SOURCE_MODEL and row.get("event_key") == EVENT_KEY and row.get("status") == "READY_UNPOSTED"),
            "customer_advance_receipt_posted_count": sum(1 for row in rows if row.get("source_model") == SOURCE_MODEL and row.get("event_key") == EVENT_KEY and row.get("status") == "POSTED"),
            "customer_advance_receipt_posted_unverified_count": sum(1 for row in rows if row.get("source_model") == SOURCE_MODEL and row.get("event_key") == EVENT_KEY and row.get("reconciliation_state") == "POSTED_UNVERIFIED"),
            "customer_advance_receipt_reconciled_count": sum(1 for row in rows if row.get("source_model") == SOURCE_MODEL and row.get("event_key") == EVENT_KEY and row.get("status") == "RECONCILED"),
            "customer_advance_receipt_blocked_count": sum(1 for row in rows if row.get("source_model") == SOURCE_MODEL and row.get("event_key") == EVENT_KEY and str(row.get("status") or "").startswith("BLOCKED")),
            "customer_advance_receipt_unsupported_count": sum(1 for row in rows if row.get("source_model") == SOURCE_MODEL and row.get("status") in {"UNSUPPORTED_SOURCE", "SKIPPED_NOT_APPLICABLE"}),
        }
    )
    return summary
