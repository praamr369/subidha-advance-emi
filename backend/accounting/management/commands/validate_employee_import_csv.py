...
from accounting.services.master_import_service import preview_employee_import


class Command(BaseCommand):
    help = "Validate an employee import CSV against the current preview-first import contract (read-only)."
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write("Employee import validation")
        ...
        if preview["errors"]:
            ...
            for row in preview["errors"][:20]:
                identifier = row.get("employee_code") or row.get("phone") or row.get("name") or "—"
                self.stdout.write(
                    f"- Row {row['row_number']} ({identifier}): {', '.join(row['errors'])}"
                )

        if fail_on_errors and preview["invalid_count"] > 0:
            raise CommandError("Employee CSV failed validation.")

        self.stdout.write("")
        self.stdout.write(self.style.SUCCESS("Employee CSV validation completed."))
