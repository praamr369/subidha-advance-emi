...
class TaxInvoice(AccountingTimeStampedModel):
    ...
    # INV-4 — IRN (Invoice Reference Number) issued by the GST e-invoice portal.
    # Once set, the invoice is e-invoice-locked and all financial fields become immutable.
    ...
    irn_generated_at = models.DateTimeField(
    ...
    def save(self, *args, **kwargs):
        ...
        # INV-3 — once a TaxInvoice leaves DRAFT status all financial and identity
        # fields are write-protected (append-only amendment model).
        if self.pk:
            existing = TaxInvoice.objects.filter(pk=self.pk).values(
                "status", "subtotal_taxable", "cgst_amount", "sgst_amount",
                "igst_amount", "total_amount", "invoice_date", "supply_kind",
                "supplier_name", "supplier_gstin", "recipient_name", "recipient_gstin",
                "doc_series_id",
            ).first()
            if existing and existing["status"] != TaxDocumentStatus.DRAFT:
                locked = {
                    "subtotal_taxable", "cgst_amount", "sgst_amount", "igst_amount",
                    "total_amount", "invoice_date", "supply_kind",
                    "supplier_name", "supplier_gstin", "recipient_name", "recipient_gstin",
                    "doc_series_id",
                }
                for field in locked:
                    if getattr(self, field) != existing[field]:
                        ...
                        raise _VE(
                            {field: f"TaxInvoice.{field} is immutable once the invoice leaves DRAFT status (INV-3)."}
                        )
        # INV-4 — once an IRN is assigned, all financial fields on this invoice are immutable.
        if self.pk and self.irn:
            existing = TaxInvoice.objects.filter(pk=self.pk).values(
                "irn", "subtotal_taxable", "cgst_amount", "sgst_amount",
                "igst_amount", "total_amount",
            ).first()
            if existing and existing["irn"] and existing["irn"] == self.irn:
                locked_fields = {
                    "subtotal_taxable", "cgst_amount", "sgst_amount",
                    "igst_amount", "total_amount",
                }
                for field in locked_fields:
                    if getattr(self, field) != existing[field]:
                        ...
                        raise ValidationError(
                            {field: f"TaxInvoice.{field} is immutable once IRN is assigned (INV-4)."}
                        )
