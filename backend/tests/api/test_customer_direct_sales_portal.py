...
class CustomerDirectSalesPortalApiTests(APITestCase):
    ...
    def test_direct_sale_summary_is_customer_scoped_and_payable_due_excludes_cancelled_and_draft(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        # Cancelled AND draft sales are excluded (only the finalized "own" sale
        # remains; the walk-in has no customer_id, cancelled/draft are filtered).
        self.assertEqual(response.data["total_direct_sale_invoices"], 1)
