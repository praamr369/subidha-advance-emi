...
from contracts.services.rent_lease_billing_service import approve_deposit_refund, record_deposit_refund
...
class AccountingBridgeSecurityDepositRefundPhaseF18Tests(APITestCase):
    def setUp(self):
        self.admin = create_admin_user(username="phase_f18_admin", phone="9305180001")
        self.cashier = create_cashier_user(username="phase_f18_cashier", phone="9305180002")
        self.customer_user = create_customer_user(username="phase_f18_customer", phone="9305180003")
        self.customer = create_customer_profile(user=self.customer_user, name="F18 Customer", phone="9305180003")
        self.product = create_product(name="F18 Sofa", product_code="F18-SOFA", base_price=Decimal("24000.00"))
        ...
        self.finance_chart_account = self.finance_account.chart_account
    ...
    def _subscription(self, *, plan_type=PlanType.RENT):
        ...
        return factory(customer=self.customer, product=self.product, tenure_months=12, start_date=date(self.today.year, self.today.month, 1), security_deposit_percent=Decimal("20.00"), performed_by=self.admin)

    def _refund(self, *, plan_type=PlanType.RENT, amount=Decimal("300.00"), suffix="001"):
        subscription = self._subscription(plan_type=plan_type)
        collect_security_deposit_with_metadata(subscription=subscription, amount=Decimal("1000.00"), performed_by=self.admin, reference_no=f"F18-RCPT-{plan_type}-{suffix}", finance_account_id=self.finance_account.id, payment_method="CASH", payment_date=self.today, idempotency_key=f"f18-receipt-{plan_type}-{suffix}")
        record_deposit_refund(subscription=subscription, amount=amount, performed_by=self.admin, reference_no=f"F18-RFND-{plan_type}-{suffix}", finance_account_id=self.finance_account.id, payment_method="CASH", payment_date=self.today, idempotency_key=f"f18-refund-{plan_type}-{suffix}")
        return RentLeaseDepositTransaction.objects.select_related("demand", "subscription", "customer", "finance_account", "finance_account__chart_account").get(external_reference_no=f"F18-RFND-{plan_type}-{suffix}")

    def _receipt(self, *, plan_type=PlanType.RENT, suffix="RCPT"):
        subscription = self._subscription(plan_type=plan_type)
        collect_security_deposit_with_metadata(subscription=subscription, amount=Decimal("1000.00"), performed_by=self.admin, reference_no=f"F18-RCPT-ONLY-{plan_type}-{suffix}", finance_account_id=self.finance_account.id, payment_method="CASH", payment_date=self.today, idempotency_key=f"f18-receipt-only-{plan_type}-{suffix}")
        return RentLeaseDepositTransaction.objects.select_related("demand", "subscription", "customer", "finance_account", "finance_account__chart_account").get(external_reference_no=f"F18-RCPT-ONLY-{plan_type}-{suffix}")

    def _candidate_id(self, tx):
        event_key = "rent_security_deposit_refund" if tx.plan_type == PlanType.RENT else "lease_security_deposit_refund"
    ...
    def _snapshot(self, tx):
        ...
        return {
            "deposit": {"amount": tx.amount, "status": tx.status, "transaction_type": tx.transaction_type, "transaction_date": tx.transaction_date, "payment_method": tx.payment_method, "finance_account_id": tx.finance_account_id},
            "demand": {"status": tx.demand.status if tx.demand_id else None, "collected_amount": tx.demand.collected_amount if tx.demand_id else None, "held_amount": tx.demand.held_amount if tx.demand_id else None, "refundable_amount": tx.demand.refundable_amount if tx.demand_id else None},
    ...
    def _run_checks(self):
        run = ReconciliationRun.objects.create(run_no=next_reconciliation_run_no(), scope="PHASE_F18_TEST", module="ACCOUNTING_BRIDGE", date_from=self.today, date_to=self.today, status=ReconciliationRunStatus.RUNNING, started_by=self.admin)
        totals = {"checked": 0, "matched": 0, "exceptions": 0, "high_risk": 0}
        run_accounting_bridge_checks(run=run, totals=totals)
        return run

    def test_concrete_rent_and_lease_refund_candidate_generation(self):
        rent = self._refund(plan_type=PlanType.RENT, suffix="RENT")
        lease = self._refund(plan_type=PlanType.LEASE, suffix="LEASE")
        ...
        self.assertEqual(rows[rent.id]["event_key"], "rent_security_deposit_refund")
        self.assertEqual(rows[lease.id]["event_key"], "lease_security_deposit_refund")
        self.assertEqual(rows[rent.id]["transaction_type"], RentLeaseDepositTransactionType.DEPOSIT_REFUND)
    ...
    def test_receipt_void_and_non_admin_are_not_refund_postable(self):
        ...
        refund = self._refund(suffix="VOID")
        refund.status = RentLeaseDepositTransactionStatus.VOIDED
        refund.voided_at = timezone.now()
        refund.voided_by = self.admin
        refund.void_reason = "test void"
        refund.save()
        response = self.client.get("/api/v1/admin/accounting/bridge-reconciliation/?source_model=RentLeaseDepositTransaction")
        rows = {int(row["source_pk"]): row for row in response.data["results"]}
        self.assertEqual(rows[receipt.id]["event_key"], "rent_security_deposit_receipt")
        self.assertNotEqual(rows[receipt.id]["event_key"], "rent_security_deposit_refund")
        self.assertEqual(rows[refund.id]["status"], "SKIPPED_NOT_APPLICABLE")
        self.assertFalse(rows[refund.id]["can_post"])
        self.client.force_authenticate(user=self.cashier)
        post = self.client.post(f"/api/v1/admin/accounting/bridge-reconciliation/candidates/{self._candidate_id(refund)}/post/", {"idempotency_key": rows[refund.id]["idempotency_key"], "confirm": True}, format="json")
        self.assertIn(post.status_code, {status.HTTP_401_UNAUTHORIZED, status.HTTP_403_FORBIDDEN})

    def test_preview_is_read_only_balanced_liability_to_finance_account(self):
        tx = self._refund(plan_type=PlanType.RENT, suffix="PREVIEW")
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        self.assertEqual(response.data["source"]["transaction_type"], RentLeaseDepositTransactionType.DEPOSIT_REFUND)
        ...
        self.assertIn("refund liability", descriptions)
    ...
    def test_post_idempotent_pending_verify_and_no_source_mutation(self):
        tx = self._refund(plan_type=PlanType.LEASE, suffix="POST")
        ...
        self.assertEqual(AccountingBridgePosting.objects.filter(source_model="RentLeaseDepositTransaction", source_id=str(tx.id), purpose="LEASE_SECURITY_DEPOSIT_REFUND").count(), 1)
    ...
    def test_mapping_finance_numbering_and_period_blockers(self):
        tx = self._refund(suffix="FIN")
        ...
        self.finance_account.save(update_fields=["is_active"])
        self.finance_chart_account.is_active = False
        self.finance_chart_account.save(update_fields=["is_active"])
        response = self.client.get("/api/v1/admin/accounting/bridge-reconciliation/?source_model=RentLeaseDepositTransaction")
        row = next(item for item in response.data["results"] if int(item["source_pk"]) == tx.id)
        self.assertEqual(row["status"], "BLOCKED_BY_FINANCE_ACCOUNT")
        self.finance_chart_account.is_active = True
        self.finance_chart_account.save(update_fields=["is_active"])
        ...
        ChartOfAccount.objects.filter(system_code="SECURITY_DEPOSIT_LIABILITY").update(is_active=True)
        tx2 = self._refund(suffix="NUMBERING")
        ...
        self.env = seed_bridge_ready_environment(self.today, performed_by=self.admin)
        self.finance_account = self.env["finance_account"]
        create_closed_accounting_period(self.today, performed_by=self.admin)
        tx3 = self._refund(suffix="PERIOD")
        response = self.client.get("/api/v1/admin/accounting/bridge-reconciliation/?source_model=RentLeaseDepositTransaction")
        row = next(item for item in response.data["results"] if int(item["source_pk"]) == tx3.id)
        self.assertEqual(row["status"], "BLOCKED_BY_PERIOD")

    def test_batch_post_and_reconciliation_diagnostics(self):
        missing_tx = self._refund(suffix="RUNMISS")
        run = self._run_checks()
        self.assertTrue(ReconciliationItem.objects.filter(run=run, source_type="RentLeaseDepositTransaction", source_id=str(missing_tx.id), exception_code="SECURITY_DEPOSIT_REFUND_MISSING_ACCOUNTING_BRIDGE_POSTING").exists())

        batch_tx = self._refund(suffix="BATCH")
        ...
        self.assertTrue(ReconciliationItem.objects.filter(run=clean_run, source_type="RentLeaseDepositTransaction", source_id=str(batch_tx.id), exception_code="SECURITY_DEPOSIT_REFUND_POSTED_UNVERIFIED").exists())

        amount_tx = self._refund(suffix="AMOUNT")
        self._post_refund(amount_tx)
        amount_journal = AccountingBridgePosting.objects.get(source_model="RentLeaseDepositTransaction", source_id=str(amount_tx.id), purpose="RENT_SECURITY_DEPOSIT_REFUND").journal_entry
        ...
        self.assertTrue(ReconciliationItem.objects.filter(run=amount_run, source_type="RentLeaseDepositTransaction", source_id=str(amount_tx.id), exception_code="SECURITY_DEPOSIT_REFUND_AMOUNT_MISMATCH").exists())

        broken_tx = self._refund(suffix="LINK")
        self._post_refund(broken_tx)
        broken_journal = AccountingBridgePosting.objects.get(source_model="RentLeaseDepositTransaction", source_id=str(broken_tx.id), purpose="RENT_SECURITY_DEPOSIT_REFUND").journal_entry
        JournalEntry.objects.filter(pk=broken_journal.pk).update(source_id="broken-source")
        link_run = self._run_checks()
        self.assertTrue(ReconciliationItem.objects.filter(run=link_run, exception_code="SECURITY_DEPOSIT_REFUND_SOURCE_LINK_MISSING").exists())

        unbalanced_tx = self._refund(suffix="UNBAL")
        self._post_refund(unbalanced_tx)
        unbalanced_journal = AccountingBridgePosting.objects.get(source_model="RentLeaseDepositTransaction", source_id=str(unbalanced_tx.id), purpose="RENT_SECURITY_DEPOSIT_REFUND").journal_entry
        ...
        self.assertTrue(ReconciliationItem.objects.filter(run=unbalanced_run, source_type="RentLeaseDepositTransaction", source_id=str(unbalanced_tx.id), exception_code="SECURITY_DEPOSIT_REFUND_JOURNAL_UNBALANCED").exists())

        duplicate_tx = self._refund(plan_type=PlanType.LEASE, suffix="DUP")
        self._post_refund(duplicate_tx)
        original = AccountingBridgePosting.objects.get(source_model="RentLeaseDepositTransaction", source_id=str(duplicate_tx.id), purpose="LEASE_SECURITY_DEPOSIT_REFUND").journal_entry
        ...
        self.assertTrue(ReconciliationItem.objects.filter(run=duplicate_run, source_type="RentLeaseDepositTransaction", source_id=str(duplicate_tx.id), exception_code="SECURITY_DEPOSIT_REFUND_DUPLICATE_ACCOUNTING_BRIDGE_POSTING").exists())
