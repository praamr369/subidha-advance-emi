...
class CustomerAdvanceRefundSourceContractPhaseF22Tests(TestCase):
    def setUp(self):
        ...
        product = create_product(name="F22 Product", product_code="F22-PROD", base_price=Decimal("3000.00"))
        batch = create_batch(batch_code="F22BATCH", duration_months=3, total_slots=100)
        lucky_id = create_lucky_id(batch=batch, lucky_number=22)
        self.subscription = create_subscription(customer=self.customer, product=product, batch=batch, lucky_id=lucky_id, total_amount=Decimal("3000.00"), monthly_amount=Decimal("1000.00"), tenure_months=3)
    ...
    def _advance(self, *, amount=Decimal("900.00"), suffix="001") -> CustomerAdvance:
        return CustomerAdvanceService.collect_unapplied_advance(customer_id=self.customer.id, amount=amount, collected_by=self.admin, finance_account_id=self.finance_account.id, method="CASH", reference_no=f"F22-ADV-{suffix}", payment_date=self.today, idempotency_key=f"f22-advance-{suffix}")

    def _counts(self):
        return {"journals": JournalEntry.objects.count(), "bridge_postings": AccountingBridgePosting.objects.count(), "reconciliation_items": ReconciliationItem.objects.count()}
    ...
    def test_refund_source_captures_required_evidence_without_accounting_records(self):
        ...
        before = self._counts()
        refund = record_customer_advance_refund(customer_advance_id=advance.id, amount=Decimal("400.00"), refunded_by=self.admin, finance_account_id=self.finance_account.id, payment_method="BANK", refund_date=self.today, refund_reference_no="F22-REF-SRC", idempotency_key="f22-ref-src", notes="Source contract test")
    ...
    def test_refund_source_listing_is_evidence_only_and_period_not_closed(self):
