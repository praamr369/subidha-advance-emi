...
from branch_control.services.import_service import preview_branch_import


class Command(BaseCommand):
    help = "Validate a branch import CSV against the current preview-first import contract (read-only)."

    def add_arguments(self, parser):
        parser.add_argument("csv_path", help="Path to the branch CSV file.")
        parser.add_argument(
            "--fail-on-errors",
            action="store_true",
    ...
    def handle(self, *args, **options):
        ...
        preview = preview_branch_import(csv_text)
        row_count = preview["valid_count"] + preview["invalid_count"]
        self.stdout.write("Branch import validation")
        ...
        if preview["errors"]:
            ...
            for row in preview["errors"][:20]:
                identifier = row.get("code") or row.get("name") or "—"
                self.stdout.write(
                    f"- Row {row['row_number']} ({identifier}): {', '.join(row['errors'])}"
                )

        if fail_on_errors and preview["invalid_count"] > 0:
            raise CommandError("Branch CSV failed validation.")

        self.stdout.write("")
        self.stdout.write(self.style.SUCCESS("Branch CSV validation completed."))
