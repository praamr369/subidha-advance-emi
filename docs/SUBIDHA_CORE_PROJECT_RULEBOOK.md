...
## 38. Financial Reporting & KPI Rules (added 2026-08-20)
...
## 39. Manufacturing Labor & Payroll Rules (added 2026-08-20)
...
## 40. Legacy Receivable & Opening Outstanding Rules (added 2026-08-20)

### Models
- `CustomerOpeningOutstanding` stores pre-system (BillBook) receivables. Has nullable `customer` FK (CRM link) and nullable `migration_row` FK (migration center traceability).
- `LegacyReceivableCollection` records each real money-in collection against a legacy receivable, with a posted journal entry (Dr cash/bank/UPI, Cr Accounts Receivable).
- Admin verification fields: `admin_verified`, `admin_verified_at`, `admin_verified_by`, `admin_verification_notes` track whether admin has confirmed each legacy balance.

### Customer linking
- Three creation paths: (A) migration-center customer import auto-links customer, (B) migration-center outstanding import uses `find_or_create_customer` by phone, (C) manual creation looks up existing Customer by phone then name.
- `sync_outstanding_customer_links()` re-links orphaned records (customer=NULL) by matching phone then name against existing CRM customers. Available via `POST /api/v1/admin/opening-balances/customers/sync/`.
- The customer profile operational view shows legacy receivable detail rows plus collection history with journal entry references.

### Financial rules
- `legacy_outstanding_amount` on customer profiles must use remaining balance (`outstanding_amount - collected_amount`) of unsettled records only, never the original amount.
- Each collection posts a journal entry: Dr chosen finance account (cash/bank/UPI), Cr Accounts Receivable (CUSTOMER_RECEIVABLE).
- Auto-settles when running balance reaches zero.
- Outstanding ledger rows must carry `customer_id`, `customer_url`, and `admin_verified` for proper navigation and verification status.

### Admin verification
- Admin can toggle verified/unverified status per opening outstanding from `/admin/opening-balances/customers` page.
- Verification is informational (does not block collection) but tracks whether admin has confirmed the legacy balance is genuine.
- Customer profile shows verified/unverified count in the legacy receivables summary.
