...
export default function PayrollBridgeReadinessPanel({ title, description, eventKeys }: Props) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!cancelled) setError(err instanceof Error ? err.message : "Failed to load payroll readiness.");
      } finally {
    ...
    <section className="rounded-xl border border-border bg-card p-4 shadow-sm">
      ...
      {!loading && !error && visibleEvents.length === 0 ? (
        <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-950">
          No matching payroll readiness events are exposed by the backend.
        </div>
      ) : null}
