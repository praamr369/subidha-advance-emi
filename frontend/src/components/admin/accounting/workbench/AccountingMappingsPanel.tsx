...
import { reopenCurrentPeriod } from "@/services/accounting";
  ...
  return (
    <div className={cx("rounded-xl border p-3 text-sm", healthStatusClass(ready))}>
      ...
      {action ? (
        <div className="mt-3">
          {action.href ? (
            <Link href={action.href} className="inline-flex rounded-lg border border-current/20 bg-card px-2.5 py-1 text-[11px] font-semibold">{action.label}</Link>
          ) : (
            <button type="button" onClick={action.onClick} className="inline-flex rounded-lg border border-current/20 bg-card px-2.5 py-1 text-[11px] font-semibold">{action.label}</button>
          )}
        </div>
      ) : null}
    </div>
...
export function AccountingMappingsPanel() {
  ...
  const [periodReopening, setPeriodReopening] = useState(false);
  const [periodReopenNotice, setPeriodReopenNotice] = useState<string | null>(null);

  const load = useCallback(async (signal?: AbortSignal) => {
    setLoading(true);
    setError(null);
    try {
      const [healthResult, readinessResult, matrixResult, bridgeResult] = await Promise.allSettled([
        getAccountingSetupHealth({ signal }),
        getAccountingSetupReadiness({ signal }),
        getBackendAccountingSetupMatrix({ signal }),
        getRentLeasePostingBridgeConfig({ signal }),
      ]);
      if (signal?.aborted) return;
      if (healthResult.status === "fulfilled") setHealth(healthResult.value);
      if (readinessResult.status === "fulfilled") setReadiness(readinessResult.value);
      if (matrixResult.status === "fulfilled") setMatrix(matrixResult.value);
      if (bridgeResult.status === "fulfilled") setBridgeConfig(bridgeResult.value);
      const allFailed = [healthResult, readinessResult, matrixResult, bridgeResult].every((r) => r.status === "rejected");
      if (allFailed) setError("Failed to load accounting setup.");
    } catch (err) {
      if (signal?.aborted) return;
  ...
  // bridgeEnabled drives the enable/disable form (uses config.is_enabled which may be auto-ready payload)
  ...
  // bridgeApproved = mapping ready and not explicitly disabled (auto-approved by backend)
  const bridgeApproved = Boolean(bridgeReadiness?.posting_bridge_ready ?? (bridgeEnabled && bridgeMappingReady));
  // bridgeExecutionReady = bridge approved AND period controls ready (financial year + open period + journal numbering)
  const bridgeExecutionReady = Boolean(
    bridgeReadiness?.posting_execution_ready ??
      (bridgeApproved && Boolean(bridgeReadiness?.posting_controls_ready))
  );
  const bridgePeriodBlocked = bridgeApproved && !bridgeExecutionReady;
  const bridgePostingMode = bridgeReadiness?.posting_mode ?? (bridgeApproved ? "POSTING_ENABLED" : "AUDIT_DEFERRED");
  const bridgeReadyForExecution = bridgeExecutionReady;
  ...
  const postingProfileReadiness = useMemo(() => {
    return (matrix?.posting_profile_readiness ?? []).map((item) => {
      ...
      const approved = Boolean(bridgeReadiness?.posting_bridge_ready ?? (bridgeApproved && mappingReady));
      const executionReady = Boolean(bridgeReadiness?.posting_execution_ready ?? (approved && Boolean(bridgeReadiness?.posting_controls_ready)));
      const mode = bridgeReadiness?.posting_mode ?? (approved ? "POSTING_ENABLED" : "AUDIT_DEFERRED");
      ...
      return {
        ...
        operator_action: executionReady || !mappingReady ? null : "Complete Period Controls (financial year + open accounting period) to enable posting execution.",
      };
    });
  }, [bridgeApproved, bridgeMappingReady, bridgeReadiness?.posting_bridge_ready, bridgeReadiness?.posting_execution_ready, bridgeReadiness?.posting_controls_ready, bridgeReadiness?.posting_mode, matrix?.posting_profile_readiness]);
  ...
  const handleReopenCurrentPeriod = useCallback(async () => {
    setPeriodReopening(true);
    setPeriodReopenNotice(null);
    setError(null);
    try {
      const result = await reopenCurrentPeriod("Reopen current period from accounting setup bridge panel.");
      setPeriodReopenNotice(result.detail);
      await load();
    } catch (err) {
      setError(err instanceof Error ? err.message : "Failed to reopen current accounting period.");
    } finally {
      setPeriodReopening(false);
    }
  }, [load]);
  ...
  const guidedSections = [
    ...
    { title: "Expense accounts", explanation: "Delivery expense, salary/payroll, and waiver expense postings keep operational costs properly separated from revenue.", required: ["Delivery Expense", "Salary / Payroll", "EMI Waiver"], accounts: [], profiles: postingProfileReadiness.filter((item) => profileMatches(item, ["delivery_expense", "salary_payroll", "waiver_expense"])) },
      ...
      <div className="space-y-6">
        ...
        <section className="rounded-xl border border-border bg-card p-5 shadow-sm">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div><div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Operator-proof rules setup</div><h2 className="mt-1 text-xl font-semibold text-foreground">Status {displayStatus}</h2><p className="mt-2 max-w-4xl text-sm leading-6 text-muted-foreground">Bank & Cash Accounts are where money is received or paid. Posting Rules decide which ledger categories are debited and credited. Income & Expense Categories are the ledger structure. Rent/lease posting approval is separate and explicit.</p></div>
        ...
        <section id="rent-lease-posting-bridge" className="rounded-xl border border-border bg-card p-5 shadow-sm scroll-mt-24"><div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div><div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Rent / lease daily approvals</div><h2 className="mt-1 text-lg font-semibold text-foreground">Current mode {bridgePostingMode}</h2><p className="mt-2 max-w-4xl text-sm leading-6 text-muted-foreground">Enabling this approval only allows future explicit backend posting preview/execute workflows to proceed. It does not create backdated journals, payments, receipts, settlement allocations, or reconciliation records.</p></div><span className={cx("rounded-full border px-3 py-1 text-xs font-semibold", statusClass(bridgeExecutionReady ? "READY" : bridgeApproved ? "DEFERRED" : bridgeMappingReady ? "DEFERRED" : "BLOCKED"))}>{bridgeExecutionReady ? "Posting enabled" : bridgeApproved ? "Approval ready" : "Approval required"}</span></div><div className="mt-4 grid gap-3 text-sm md:grid-cols-3"><div className="rounded-xl border border-border bg-muted/20 p-3"><div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Rules readiness</div><div className="mt-1 font-semibold text-foreground">{bridgeReadiness?.status ?? "Not loaded"}</div><div className="mt-1 text-xs text-muted-foreground">{bridgeMappingReady ? "Categories and Bank Accounts are valid." : bridgeReadiness?.reason ?? "Complete mapping before enabling."}</div></div><div className="rounded-xl border border-border bg-muted/20 p-3"><div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Posting approval</div><div className="mt-1 font-semibold text-foreground">{bridgeApproved ? "Approved" : bridgeEnabled ? "Enabled" : "Disabled"}</div><div className="mt-1 text-xs text-muted-foreground">{bridgeConfig?.config?.reason || (bridgeApproved ? "Auto-approved — mapping is valid." : "No approval reason recorded.")}</div></div><div className="rounded-xl border border-border bg-muted/20 p-3"><div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Future execution</div><div className="mt-1 font-semibold text-foreground">{bridgeExecutionReady ? "Allowed" : bridgePeriodBlocked ? "Blocked — period controls" : "Blocked"}</div><div className="mt-1 text-xs text-muted-foreground">{bridgeExecutionReady ? "Future explicit posting execution is enabled." : bridgePeriodBlocked ? "Approval is ready. Set up an active financial year and an open accounting period to unblock." : bridgeReadiness?.operator_action || "Future explicit posting execution is blocked."}</div>{bridgePeriodBlocked ? <Link href={ROUTES.admin.accountingPeriods} className="mt-2 inline-block rounded-lg border border-amber-300 bg-amber-50 px-2.5 py-1 text-[11px] font-semibold text-amber-900 hover:bg-amber-100">Open Period Controls →</Link> : null}</div></div>{bridgeReadiness?.blockers?.length ? <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-950">{bridgeReadiness.blockers[0]}</div> : null}
          {bridgeReadiness?.status === "NEEDS_ACCOUNTING_PERIOD" && bridgeReadiness?.mapping_ready ? (
            <div className="mt-4 rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-900">
              <div className="font-semibold">Current accounting period is locked — bridge cannot be enabled</div>
              <p className="mt-1 text-xs">The rent/lease COA and finance account mapping is valid, but the current period is locked. Posting requires an OPEN period.</p>
              {periodReopenNotice ? <p className="mt-2 text-xs font-semibold text-emerald-700">{periodReopenNotice}</p> : null}
              <button type="button" disabled={periodReopening} onClick={() => void handleReopenCurrentPeriod()} className="mt-2 rounded-lg border border-red-300 bg-white px-3 py-1.5 text-xs font-semibold text-red-900 hover:bg-red-50 disabled:opacity-50">{periodReopening ? "Reopening…" : "Reopen Current Period"}</button>
            </div>
          ) : null}
          <div className="mt-4 grid gap-3 lg:grid-cols-[1.2fr_1fr]"><label className="text-sm"><div className="mb-1 text-xs font-semibold text-muted-foreground">Reason</div><textarea className="min-h-24 w-full rounded-xl border border-border bg-background px-3 py-2 text-sm" value={bridgeReason} onChange={(event) => setBridgeReason(event.target.value)} placeholder={bridgeEnabled ? "Reason for disabling posting bridge" : "Reason for enabling posting bridge"} /></label><div className="space-y-3"><label className="block text-sm"><div className="mb-1 text-xs font-semibold text-muted-foreground">Typed confirmation</div><input className="w-full rounded-xl border border-border bg-background px-3 py-2 text-sm" value={bridgeConfirmation} onChange={(event) => setBridgeConfirmation(event.target.value)} placeholder={expectedBridgeConfirmation} /></label><div className="rounded-xl border border-border bg-muted/20 p-3 text-xs text-muted-foreground">Required phrase: <span className="font-semibold text-foreground">{expectedBridgeConfirmation}</span></div>{!bridgeEnabled && !bridgeMappingReady ? <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs text-amber-950">Enable is disabled until rent/lease mapping readiness is valid.</div> : null}<ActionButton variant={bridgeEnabled ? "secondary" : "primary"} onClick={() => void submitBridgeApproval()} disabled={!canSubmitBridge || bridgeSaving}>{bridgeSaving ? "Saving..." : bridgeEnabled ? "Disable posting bridge" : "Enable posting bridge"}</ActionButton></div></div></section>

        <section id="business-finance-accounts" className="space-y-3"><div><h2 className="text-lg font-semibold text-foreground">1. Business Bank & Cash Accounts</h2><p className="mt-1 text-sm text-muted-foreground">Cash desks, bank accounts, UPI accounts, and payment gateway accounts are operational money destinations. Ledger posting rules are excluded from this section.</p></div><FinanceAccountMappingPanel financeAccounts={businessFinanceAccounts} chartAccounts={chartAccounts} saving={saving || repairing} editingId={editingFinanceAccount?.id ?? null} repairId={repairing && repairTargets.length === 1 ? repairTargets[0].id : null} selectedChartAccountId={selectedChartAccountId} onEdit={openFinanceMappingEdit} onCancelEdit={() => { setEditingFinanceAccount(null); setSelectedChartAccountId(""); }} onChartAccountChange={setSelectedChartAccountId} onSave={() => void submitFinanceMappingEdit()} onRepair={(account) => void openRepairDialog([account])} onRepairAll={(accounts) => void openRepairDialog(accounts)} /></section>

        <section className="space-y-3"><div><h2 className="text-lg font-semibold text-foreground">2. System Posting Rules</h2><p className="mt-1 text-sm text-muted-foreground">System posting rules are diagnostic only and cannot receive customer collections. They decide which ledger categories are debited and credited.</p></div>{diagnosticAccounts.length > 0 ? <div className="rounded-xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-950"><div className="font-semibold">Ledger posting rules (system)</div><div className="mt-1">Diagnostic only. Not selectable for customer collections.</div><div className="mt-2 flex flex-wrap gap-2 text-xs">{diagnosticAccounts.map((account) => <span key={account.id} className="rounded-full border border-blue-200 bg-card px-2 py-1">{account.name}</span>)}</div></div> : null}<div className="grid gap-4 lg:grid-cols-2">{postingProfileReadiness.map((item) => <article key={item.key} className="rounded-xl border border-border bg-card p-4 shadow-sm"><div className="flex items-start justify-between gap-3"><div><h3 className="text-sm font-semibold text-foreground">{item.label}</h3><p className="mt-1 text-xs text-muted-foreground">{item.key}</p></div><span className={cx("rounded-full border px-2 py-1 text-xs font-semibold", statusClass(item.status))}>{item.status}</span></div><div className="mt-3 grid gap-3 text-xs md:grid-cols-2"><div className="rounded-xl border border-border bg-muted/20 p-3"><div className="font-semibold text-foreground">Debit category</div><div className="mt-1 text-muted-foreground">Required: {item.required_debit_account.join(", ") || "Not configured"}</div><div className="mt-1 text-foreground">{item.configured_debit_account.map(accountLabel).join(", ") || "Not configured"}</div></div><div className="rounded-xl border border-border bg-muted/20 p-3"><div className="font-semibold text-foreground">Credit category</div><div className="mt-1 text-muted-foreground">Required: {item.required_credit_account.join(", ") || "Not configured"}</div><div className="mt-1 text-foreground">{item.configured_credit_account.map(accountLabel).join(", ") || "Not configured"}</div></div></div><div className="mt-3 rounded-xl border border-border bg-background p-3 text-xs text-muted-foreground">{firstBlocker(item)}</div>{item.collection_ready !== undefined || item.mapping_ready !== undefined || item.posting_mode ? <div className="mt-3 grid gap-2 text-xs md:grid-cols-3"><div className="rounded-xl border border-border bg-muted/20 p-3"><div className="font-semibold text-foreground">Source collection</div><div className="mt-1 text-muted-foreground">{setupFlagLabel(item.collection_ready)}</div></div><div className="rounded-xl border border-border bg-muted/20 p-3"><div className="font-semibold text-foreground">Category / Bank mapping</div><div className="mt-1 text-muted-foreground">{setupFlagLabel(item.mapping_ready)}</div></div><div className="rounded-xl border border-border bg-muted/20 p-3"><div className="font-semibold text-foreground">Posting mode</div><div className="mt-1 text-muted-foreground">{item.posting_mode || "Not exposed"}</div></div></div> : null}{item.operator_action ? <div className="mt-2 rounded-xl border border-blue-200 bg-blue-50 p-3 text-xs text-blue-950"><div>Action: {item.operator_action}</div>{isRentLeaseBridgeProfile(item) ? <button type="button" onClick={scrollToBridgeApproval} className="mt-2 rounded-lg border border-blue-200 bg-card px-2.5 py-1 text-[11px] font-semibold text-blue-900">Open daily approvals</button> : null}</div> : null}{!item.implemented ? <div className="mt-2 text-xs font-medium text-blue-800">Deferred workflow. Do not create fake collection action.</div> : null}</article>)}</div></section>

        <section className="space-y-3"><div><h2 className="text-lg font-semibold text-foreground">3. Income & Expense Categories Health</h2><p className="mt-1 text-sm text-muted-foreground">Income & Expense Categories are the ledger structure. Collection accounts must point to active manual-entry-enabled leaf ASSET categories, not group/control categories.</p></div><div className="grid gap-3 md:grid-cols-4"><div className="rounded-xl border border-border bg-card p-4"><div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Group/control categories</div><div className="mt-2 text-2xl font-semibold text-foreground">{coaHealth?.counts?.group_control_count ?? 0}</div></div><div className="rounded-xl border border-border bg-card p-4"><div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Manual entry categories</div><div className="mt-2 text-2xl font-semibold text-foreground">{coaHealth?.counts?.posting_leaf_count ?? 0}</div></div><div className="rounded-xl border border-border bg-card p-4"><div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Missing leaf assets</div><div className="mt-2 text-2xl font-semibold text-foreground">{coaHealth?.counts?.missing_posting_leaf_count ?? 0}</div></div><div className="rounded-xl border border-border bg-card p-4"><div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Inactive/control</div><div className="mt-2 text-2xl font-semibold text-foreground">{coaHealth?.counts?.inactive_or_non_posting_count ?? 0}</div></div></div><div className="rounded-xl border border-border bg-card p-4"><div className="text-sm font-semibold text-foreground">Category blockers</div>{(coaHealth?.inactive_or_non_posting_blockers ?? []).length === 0 ? <div className="mt-2 text-sm text-emerald-700">No inactive/control blockers exposed.</div> : <div className="mt-3 overflow-x-auto"><table className="min-w-full text-left text-xs"><thead className="text-muted-foreground"><tr><th className="px-2 py-2">Code</th><th className="px-2 py-2">Name</th><th className="px-2 py-2">Type</th><th className="px-2 py-2">Issue</th></tr></thead><tbody>{(coaHealth?.inactive_or_non_posting_blockers ?? []).slice(0, 12).map((account) => <tr key={`${account.id}-${account.code}`} className="border-t border-border"><td className="px-2 py-2 font-medium text-foreground">{account.code}</td><td className="px-2 py-2">{account.name}</td><td className="px-2 py-2">{account.account_type || account.type}</td><td className="px-2 py-2 text-amber-700">{!account.is_active ? "Inactive" : account.is_group_control ? "Group/control or manual-entry disabled" : "Not payment-ready"}</td></tr>)}</tbody></table></div>}</div></section>
      </div>
    </div>
  );
}
