...
export function PublicPolicyGovernance() {
              ...
              <tbody>
                {visiblePolicies.map((row) => (
                  <tr key={row.id} className="border-b border-border/60">
                    ...
                    <td className="px-3 py-2"><Link href={`${ROUTES.admin.settingsCompliancePolicies}/${row.slug}`} className="rounded-lg border border-border bg-background px-3 py-1.5 text-xs font-semibold text-foreground transition hover:bg-accent">Open</Link></td>
                  </tr>
                ))}
              </tbody>
            </table>
