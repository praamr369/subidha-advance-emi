...
export default function StaffDataPage<T>({ title, description, load, render, empty, emptyMessage }: StaffDataPageProps<T>) {
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="rounded-xl border border-border bg-card p-5 text-sm text-muted-foreground shadow-sm">Loading...</div>
      ) : null}

      {!loading && error ? (
        <div className="rounded-xl border border-red-200 bg-red-50 p-5 text-sm text-red-900 shadow-sm">{error}</div>
      ) : null}
