...
type PartyPanel = { title: string; name?: string | null; phone?: string | null; email?: string | null; address?: string | null; gstin?: string | null };
type AmountRow = { label: string; value: ReactNode; strong?: boolean; danger?: boolean };
...
const ThemeContext = createContext<DocumentBrandTheme>(subidhaDocumentTheme);
export function useDocumentTheme() { return useContext(ThemeContext); }

function initials(name: string): string {
  return name.split(/\s+/).filter(Boolean).slice(0, 2).map((part) => part[0]).join("").toUpperCase() || "SF";
}

function inferredTermsKey(): DocumentTermsKey | null {
  if (typeof window === "undefined") return null;
  const path = window.location.pathname;
  if (path.includes("/billing/direct-sale/") && path.endsWith("/print")) return "invoice_terms";
  if (path.includes("/billing/receipts/") && path.endsWith("/print")) return "receipt_terms";
  if (path.includes("/deliveries/direct-sale-cases/") && path.endsWith("/print")) return "delivery_challan_terms";
  if (path.includes("/rent-lease/contracts/") && path.endsWith("/contract/print")) return "rent_lease_contract_terms";
  if (path.includes("/subscriptions/") && path.endsWith("/contract/print")) return "subscription_contract_terms";
  if (path.includes("/purchases/") && path.endsWith("/bill/print")) return "purchase_bill_terms";
  if (path.includes("/vendors/payments/") && path.endsWith("/voucher/print")) return "vendor_voucher_terms";
  if (path.includes("/statement/print")) return "account_statement_terms";
  return null;
}

export function DocumentPage({ children, watermark }: { children: ReactNode; watermark?: string | null }) {
  const [theme, setTheme] = useState<DocumentBrandTheme>(subidhaDocumentTheme);

  useEffect(() => {
    let mounted = true;
    getDocumentPrintSettings()
      .then((settings) => mounted && setTheme(documentThemeFromSettings(settings)))
      .catch(() => mounted && setTheme(subidhaDocumentTheme));
    return () => { mounted = false; };
  }, []);

  const resolvedWatermark = theme.showWatermark ? watermark : null;
  const padding = theme.layoutDensity === "COMPACT" ? "p-6 print:p-0" : "p-8 print:p-0";
    ...
    <ThemeContext.Provider value={theme}>
      <main className="document-screen fixed inset-0 z-[1000] min-h-screen overflow-y-auto bg-[#f4eadb] px-4 pb-8 pt-24 print:static print:z-auto print:overflow-visible print:bg-white print:p-0">
        <style jsx global>{`
          ...
          .document-screen-only { display: block; }
          @media print {
            html, body { width: 210mm; min-height: 297mm; background: #fff !important; }
            .print-toolbar, .document-screen-only, header:not(.document-header), nav, aside, [data-document-link-strip], [data-dashboard-shell], [data-dashboard-sidebar], [data-dashboard-topbar], [data-operational-action], [data-print-hidden] { display: none !important; visibility: hidden !important; }
            body * { visibility: hidden !important; }
            .document-screen, .document-screen *, .print-document, .print-document * { visibility: visible !important; }
            .document-screen { display: block !important; position: static !important; inset: auto !important; width: auto !important; min-height: auto !important; overflow: visible !important; background: #fff !important; }
            .print-document { position: static !important; width: 186mm !important; max-width: 186mm !important; margin: 0 auto !important; box-shadow: none !important; color: #2f2418 !important; background: #fff !important; }
            .document-no-break, .document-card, .document-signature, .document-amount-summary, tr { break-inside: avoid; page-break-inside: avoid; }
            thead { display: table-header-group; }
            tfoot { display: table-footer-group; }
            th { color: #2f2418 !important; background: #f5ead8 !important; }
            .document-card, .document-amount-summary, .document-signature { background: #fff !important; border-color: #d9c39c !important; }
            .document-print-footer { position: fixed; bottom: 0; left: 0; right: 0; background: #fff !important; }
...
export function DocumentHeader({ copyLabel, documentNo, documentDate }: { copyLabel: DocumentCopyLabel; documentNo?: string | null; documentDate?: string | null }) {
    ...
    <header className="document-header document-no-break border-b-4 border-[#8a5a22] pb-5">
      <div className="flex items-start justify-between gap-6">
        <div className="flex items-start gap-4">
          ...
          <div>
            <div className="text-2xl font-black uppercase tracking-[0.08em] text-[#5e3818]">{theme.businessName}</div>
            {theme.tagline ? <div className="mt-0.5 text-xs font-semibold uppercase tracking-[0.14em] text-[#8a5a22]">{theme.tagline}</div> : null}
            <div className="mt-1 whitespace-pre-line text-sm leading-5 text-[#6f5c46]">{theme.addressLines.join("\n")}</div>
            <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs font-medium text-[#6f5c46]">
              ...
              {theme.taxLabel ? <span>{theme.taxLabel}</span> : null}
            </div>
          </div>
        </div>
...
export function DocumentTermsBlock({ terms, termsKey }: { terms?: string[]; termsKey?: DocumentTermsKey }) {
  ...
  const finalTerms = useMemo(() => {
    const dynamicTerms = termsForDocument(theme, termsKey || inferredTermsKey());
...
export function DocumentSignatureBlock({ labels }: { labels: string[] }) {
  const theme = useDocumentTheme();
  const finalLabels = labels.map((label) => /Authorized/i.test(label) ? theme.signatureLabels.authorized : /Customer/i.test(label) ? theme.signatureLabels.customer : label);
  return <section className="document-no-break mt-8 grid gap-6 sm:grid-cols-2">{finalLabels.map((label) => <div key={label} className="document-signature pt-10 text-center"><div className="border-t border-[#8a7255] pt-2 text-xs font-bold uppercase tracking-[0.12em] text-[#6f4e27]">{label}</div></div>)}</section>;
}
