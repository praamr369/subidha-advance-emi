...
import { RefreshCw, Search, Wallet, ChevronRight } from "lucide-react";
...
export default function PartnerCommissionsPage() {
  ...
  const loadCommissions = useCallback(async (mode: "initial" | "refresh" = "initial") => {
    ...
    <div className="flex flex-col p-4 space-y-5">
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Commission entries and ledger
          </p>
        </div>
        <button
          type="button"
          onClick={() => void loadCommissions("refresh")}
          disabled={refreshing}
      ...
      <div className="flex items-center gap-3 rounded-2xl border border-border bg-card p-4 shadow-sm">
        ...
        <div>
          ...
          <div className="text-xs font-medium text-muted-foreground">Total Entries</div>
        </div>
      </div>
      ...
      <div className="space-y-3">
        {loading ? (
          ...
          <ErrorState title="Error" description={error} onRetry={() => void loadCommissions("initial")} />
        ) : count === 0 ? (
          <EmptyState
            title="No commissions found"
            description={appliedSearch || appliedStatus ? "No entries matched your filters." : "You have no commission entries."}
          />
        ) : (
          rows.map((row) => {
