...
function BlockerList({ items, emptyLabel }: { items?: CloseCockpitBlocker[] | null; emptyLabel: string }) {
  ...
  return (
    <ul className="space-y-2">
      {safeItems.map((b, index) => (
        <li key={`${b.key}-${index}`} className="flex items-start gap-2 rounded-lg border border-border bg-muted/40 px-3 py-2 text-sm">
          ...
          {b.action_url && (
            <Link href={b.action_url} className="ml-auto flex-shrink-0 rounded-md border border-border bg-background px-3 py-1 text-xs font-semibold text-foreground hover:bg-muted">
              Resolve
            </Link>
          )}
        </li>
  ...
  return (
    <ul className="space-y-2">
      {safeItems.map((item, index) => (
        <li key={`${item.key}-${index}`} className="flex items-start gap-2 rounded-lg border border-border bg-background px-3 py-2 text-sm">
          ...
          <div className="ml-auto flex flex-col items-end justify-center gap-2">
            ...
            {item.action_url && (
              <Link href={item.action_url} className="whitespace-nowrap rounded-md border border-border bg-background px-3 py-1 text-xs font-semibold text-foreground hover:bg-muted">
                Resolve
              </Link>
            )}
          </div>
...
export default function AccountingCloseCockpitPage() {
  ...
  async function handleLockPeriod() {
    if (!cockpit?.sections?.period_lock?.period_id) return;
    ...
    try {
      await lockAccountingPeriod(cockpit.sections.period_lock.period_id, "Locked from Month-End Closing Dashboard");
      setActionSuccess("Period locked successfully.");
      await load();
    } catch (err) {
      setActionError(err instanceof Error ? err.message : "Failed to lock period.");
    } finally {
  ...
  async function handleClosePeriod() {
    if (!cockpit?.sections?.period_lock?.period_id) return;
    if (!confirm("Are you sure you want to officially close this period?")) return;
    
    setActionLoading(true);
    setActionError(null);
    setActionSuccess(null);
    try {
      await closeAccountingPeriod(cockpit.sections.period_lock.period_id, "Closed from Month-End Closing Dashboard");
      setActionSuccess("Period closed successfully.");
      await load();
    } catch (err) {
  ...
  return (
    <ERPPageShell
      ...
      title="Month-End Closing Dashboard"
      subtitle="Verify all system checks to ensure the month is ready to be securely closed and locked."
      helperNote="When all checks pass, you can officially Lock or Close the month directly from this dashboard."
      helperTone="info"
      breadcrumbs={[
        { label: "Admin", href: ROUTES.admin.dashboard },
        { label: "Accounting", href: ROUTES.admin.accounting },
        { label: "Month-End Closing Dashboard" },
      ]}
      actions={[
    ...
    >
      <div className="mb-6 rounded-xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-900">
        ...
        <ol className="mt-1 ml-4 list-decimal space-y-1">
          <li>Select the Month and Year you wish to close.</li>
          <li>Review the <strong>Blockers</strong> section. All critical blockers must be resolved before the system will allow you to close.</li>
          <li>Once <em>Can Close</em> or <em>Can Lock</em> shows <strong>YES</strong>, you can use the action buttons to officially lock or close the books.</li>
        </ol>
      </div>
      {/* Period selector */}
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-xl border border-border bg-background p-5 shadow-sm">
            ...
            {(actionError || actionSuccess) && (
              <div className={`mt-4 rounded-lg border px-3 py-2 text-sm ${actionError ? "border-red-200 bg-red-50 text-red-800" : "border-green-200 bg-green-50 text-green-800"}`}>
                {actionError || actionSuccess}
              </div>
            )}
            ...
            <div className="mt-4 flex gap-3">
              {cockpit.can_lock && cockpit.sections.period_lock.period_id && (
                ...
                >
                  {actionLoading ? "Processing..." : `Lock Period ${cockpit.period_state.period_code}`}
                </button>
              )}
              {cockpit.can_close && cockpit.sections.period_lock.period_id && (
                <button
                ...
                >
                  {actionLoading ? "Processing..." : `Close Period ${cockpit.period_state.period_code}`}
                </button>
              )}
            </div>
          </div>
          ...
          <ERPSectionShell
            title="System check readiness"
            description="Status of each background system check. Expand individual sections for detail."
          >
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
              <SectionCard title="Month-End Close Checks" section={cockpit.sections.month_end} />
              <SectionCard title="Diagnostics Checks" section={cockpit.sections.financial_intelligence} />
              <SectionCard title="Trial Balance Checks" section={cockpit.sections.trial_balance} />
              <SectionCard title="Deposit & Liability Checks" section={cockpit.sections.liability_reconciliation} />
              <SectionCard title="Period Lock Status" section={{ status: cockpit.sections.period_lock.period_exists ? (cockpit.sections.period_lock.is_closed ? "CLOSED" : cockpit.sections.period_lock.is_locked ? "LOCKED" : "OPEN") : "MISSING" }} />
            </div>
          </ERPSectionShell>
