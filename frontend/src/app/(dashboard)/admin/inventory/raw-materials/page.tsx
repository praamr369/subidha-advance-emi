...
import { listRawMaterials, type RawMaterialRow } from "@/services/inventory";
...
export default function RawMaterialsPage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const payload = await listRawMaterials({ q: search, page, page_size: PAGE_SIZE });
        if (cancelled) return;
        ...
        if (cancelled) return;
        setError(err instanceof Error ? err.message : "Failed to load raw materials.");
      } finally {
  ...
  return (
    <ERPPageShell
      eyebrow="Inventory"
      title="Raw Materials"
      subtitle="Input materials used in manufacturing BOMs. Each item here can appear as a BOM line for finished goods."
      breadcrumbs={[
      ...
      actions={[
        ...
        { href: ROUTES.admin.manufacturingBoms, label: "BOMs", variant: "secondary" },
      ]}
    >
      ...
      <ERPSectionShell title="Raw Materials Register" description="Products configured as raw-material inventory items. Used in BOM lines.">
        {loading ? <ERPLoadingState label="Loading raw materials…" /> : null}
        {!loading && error ? <ERPErrorState title="Unable to load" description={error} /> : null}
        {!loading && !error && rows.length === 0 ? (
          <ERPEmptyState
            title="No raw materials found"
            description="Create products and set their inventory profile type to Raw Material."
          />
        ) : null}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border text-xs font-semibold uppercase tracking-wider text-muted-foreground">
                  ...
                  <th className="pb-2 text-right">BOM Uses</th>
                ...
                {rows.map((row) => (
                  <tr key={row.id} className="border-b border-border/50 hover:bg-muted/20">
                    ...
                    <td className="py-2 text-right">
                      <span className={`font-semibold ${row.bom_usage_count > 0 ? "text-green-600 dark:text-green-400" : "text-muted-foreground"}`}>
                        {row.bom_usage_count}
                      </span>
                    </td>
                    <td className="py-2">
      ...
      <ERPSectionShell title="Guidance" description="How to add a raw material">
        <div className="rounded-xl border border-border bg-muted/50 px-4 py-3 text-sm text-muted-foreground space-y-1">
          <p>1. Go to <Link href={ROUTES.admin.productsCreate} className="underline">Products → Create</Link> and add the raw material as a product.</p>
          <p>2. Open the product, then click <strong>Prepare Inventory Profile</strong> and set the type to <strong>Raw Material</strong>.</p>
          <p>3. Once profiled, it will appear here and be available as a BOM line in <Link href={ROUTES.admin.manufacturingBoms} className="underline">Manufacturing → BOMs</Link>.</p>
        </div>
      </ERPSectionShell>
    </ERPPageShell>
  );
