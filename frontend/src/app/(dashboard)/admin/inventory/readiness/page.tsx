      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {section.blockers} blocker(s), {section.warnings} warning(s)
          </p>
        </div>
        <span className={`rounded-full border px-2.5 py-1 text-xs font-semibold ${statusBadgeClass(section.status)}`}>
          {section.status}
        </span>
      </div>
            ...
            {section.checks.map((check) => (
              <tr key={check.key} className="align-top">
                ...
                <td className="py-3 pr-4">
                  <span className={`rounded-full border px-2 py-0.5 text-xs font-semibold ${statusBadgeClass(check.status)}`}>
                    {check.status}
                  </span>
                </td>
                ...
                <td className="max-w-xl py-3 pr-4 text-muted-foreground">{check.detail}</td>
                <td className="py-3">
                  {check.action_href && ALLOWED_SHORTCUTS.has(check.action_href) ? (
                    <Link className="inline-flex items-center gap-1 text-sm font-medium text-primary hover:underline" href={check.action_href}>
        ...
        <table className="min-w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-border">
            {issues.map((issue, index) => (
              <tr key={`${issue.severity}-${issue.section}-${issue.title}-${index}`} className="align-top">
                ...
                <td className="px-4 py-3">
                  {issue.action_href && ALLOWED_SHORTCUTS.has(issue.action_href) ? (
                    <Link className="inline-flex items-center gap-1 font-medium text-primary hover:underline" href={issue.action_href}>
...
export default function InventoryReadinessPage() {
  ...
  const shortcuts = useMemo(
    () => (data?.operator_shortcuts ?? []).filter((shortcut) => ALLOWED_SHORTCUTS.has(shortcut.href)),
    [data?.operator_shortcuts]
  );
  ...
  return (
    <ERPPageShell
      ...
      subtitle="Read-only operational cockpit for stock, delivery, returns, finance mapping, and reconciliation readiness."
      breadcrumbs={[
      ...
      <div className="space-y-6">
        ...
        {!loading && error ? (
          <ERPErrorState
            ...
            description={`${error} Check API connectivity and retry this read-only page.`}
          />
        ) : null}
        ...
        {!loading && !error && data && !data.module_not_configured ? (
          <>
                ...
                <div className="flex min-w-0 items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-2 max-w-3xl text-sm">
                      This snapshot is generated from persisted product, stock, delivery, purchase, and accounting setup records. It does not post stock, mutate delivery status, create accounting entries, or reconcile records.
                    </p>
                  </div>
                </div>
                <div className="inline-flex items-center gap-2 rounded-full border border-current/20 px-3 py-1 text-sm">
            ...
            <ERPSectionShell title="Issues" description="Grouped readiness issues without customer PII.">
              {data.issues.length === 0 ? (
                <ERPEmptyState
                  title="No readiness issues"
                  description="All inventory readiness checks returned ready. This is a valid ready state, not missing data."
                />
              ) : (
                <div className="space-y-4">
                  <IssueGroup title={`Blockers (${issueGroups.blockers.length})`} issues={issueGroups.blockers} />
                  <IssueGroup title={`Warnings (${issueGroups.warnings.length})`} issues={issueGroups.warnings} />
                  <IssueGroup title={`Info (${issueGroups.info.length})`} issues={issueGroups.info} />
                </div>
              )}
            </ERPSectionShell>
