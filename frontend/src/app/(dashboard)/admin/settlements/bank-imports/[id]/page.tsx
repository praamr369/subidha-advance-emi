// src/app/(dashboard)/admin/settlements/bank-imports/[id]/page.tsx
...
export default function BankImportDetail({ params }: { params: { id: string } }) {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const imp = await getBankImport(importId);
      const resp = await listBankImportLines(importId);
      setImportData(imp);
  ...
  const handleAllocate = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const payload: SettlementAllocationCreatePayload = {
      source_type: "BANK_STATEMENT_LINE",
      source_id,
      finance_account: importData?.bank_finance_account ?? 0,
      matched_amount,
      note: note || undefined,
    };
  ...
  if (loading) return <ERPLoadingState label="Loading bank import..." />;
  if (error) return <ERPErrorState title="Bank import unavailable" description={error} onRetry={() => void fetchData()} />;
  ...
  return (
    <ERPPageShell
      ...
      breadcrumbs={[
        ...
        { label: "Bank Imports", href: ROUTES.admin.settlementsBankImports },
    ...
    >
      <div className="space-y-6">
          ...
          <ERPDetailGrid
            items={[
              { label: "Import no", value: importData.import_no },
              {
                ...
                value: importData.bank_finance_account_name ?? `#${importData.bank_finance_account}`,
                hint: `ID ${importData.bank_finance_account}`,
              },
              { label: "Statement period", value: `${importData.statement_period_from} → ${importData.statement_period_to}` },
        ...
        <ERPSectionShell title="Statement lines" description="Parsed statement rows. Matched status is derived from allocations.">
          {lines.length === 0 ? (
            <ERPEmptyState title="No lines found" description="This import has no parsed statement lines." />
          ) : (
              ...
              <tbody>
                {lines.map((line) => (
                  <tr key={line.id} className="border-t border-border/60">
                    <td className="p-3 text-sm text-foreground">{line.transaction_date}</td>
                    <td className="p-3 text-sm text-foreground">{line.description}</td>
                    <td className="p-3 text-sm text-muted-foreground">{line.reference_no ?? "—"}</td>
                    <td className="p-3 text-right text-sm text-foreground">{line.debit}</td>
                    <td className="p-3 text-right text-sm text-foreground">{line.credit}</td>
        ...
        <ERPSectionShell
          ...
          description="Create an explicit allocation linking one statement line to exactly one target type."
        >
          <ERPDataToolbar
            left={
              <div className="text-sm text-muted-foreground">
                ...
                <span className="font-semibold text-foreground">
                  {importData.bank_finance_account_name ?? `#${importData.bank_finance_account}`}
                </span>
              </div>
            }
          />
                ...
                >
                  ...
                  {lines.map((line) => (
                    <option key={line.id} value={String(line.id)}>
                      #{line.id} • {line.transaction_date} • {line.description.slice(0, 64)} • Cr {line.credit} / Dr {line.debit}
                    </option>
                  ))}
                </select>
              </label>
                        ...
                        <td className="p-3 text-right text-sm">
                          {row.status !== "VOIDED" ? (
                            <ConfirmActionButton
                              ...
                              variant="destructive"
                              onConfirm={async () => {
                                await voidAllocation(row.id, { reason: "Voided from bank import evidence page." });
                                await Promise.all([
                                  loadAllocationsForLine(selectedLineId),
                                  (async () => {
                                    const refreshedLines = await listBankImportLines(importId);
                                    setLines(refreshedLines.results ?? []);
                                  })(),
                                ]);
                              }}
