          ...
          <div className="grid gap-3 md:grid-cols-3">
            ...
            <Field label="Probation end date">
              <div className="flex gap-2">
                <SelectInput value="" onChange={(val) => {
                  if (!form.joining_date) return alert("Please set Joining date in Step 1 first");
                  if (!val) return;
                  const jd = new Date(form.joining_date);
                  if (val === "1M") jd.setMonth(jd.getMonth() + 1);
                  else if (val === "3M") jd.setMonth(jd.getMonth() + 3);
                  else if (val === "6M") jd.setMonth(jd.getMonth() + 6);
                  else if (val === "1Y") jd.setFullYear(jd.getFullYear() + 1);
                  onChange({ ...form, probation_end_date: jd.toISOString().split('T')[0] });
                }}>
                ...
                <div className="flex-1">
                  <TextInput type="date" value={form.probation_end_date} onChange={(value) => update("probation_end_date", value)} />
                </div>
              </div>
            </Field>
