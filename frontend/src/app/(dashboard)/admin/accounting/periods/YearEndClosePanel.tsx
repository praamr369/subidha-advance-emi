...
function remediationSteps(readiness: YearEndReadiness | null) {
  ...
  return [
    ...
    { label: "Review unposted bridge items", ok: unposted === 0, detail: unposted ? `${unposted} READY_UNPOSTED row(s)` : "No unposted bridge blockers", href: `${ROUTES.admin.accountingBridgeReconciliation}?${new URLSearchParams({ ...fy, status: "READY_UNPOSTED" }).toString()}` },
    { label: "Fix blocked bridge mappings", ok: blocked === 0, detail: blocked ? `${blocked} blocked mapping warning(s)` : "No blocked mapping warnings", href: `${ROUTES.admin.accountingBridgeReconciliation}?${new URLSearchParams({ ...fy, status: "BLOCKED_BY_MAPPING" }).toString()}` },
    ...
    { label: "Lock eligible periods", ok: open === 0, detail: open ? "Use period table Lock buttons" : "No open period blocker", href: ROUTES.admin.accountingPeriods },
...
function closeBlockReason(readiness: YearEndReadiness | null): string {
  ...
  if (readiness.open_period_count && readiness.unposted_bridge_item_count) return "Cannot close: open periods and unposted bridge items remain.";
  const parts: string[] = [];
  if (readiness.open_period_count) parts.push("open periods remain");
  if (readiness.unposted_bridge_item_count) parts.push("unposted bridge items remain");
  if (readiness.blocked_bridge_item_count) parts.push("blocked bridge mappings remain");
  if (readiness.missing_period_count) parts.push("missing periods remain");
  if (readiness.gap_or_overlap_count) parts.push("period date gaps/overlaps remain");
  if (readiness.missing_numbering_profile_count) parts.push("numbering profile issues remain");
