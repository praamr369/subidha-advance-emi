...
const SOURCE_MODEL_OPTIONS = ["", "Payment", "ReceiptDocument", "BillingInvoice", "BillingCreditNote", "DirectSaleReturn", "BillingDebitNote", "PurchaseBill", "VendorPayment", "StockLedger"];
const CONCRETE_POST_MODELS = new Set(["Payment", "ReceiptDocument", "BillingInvoice", "BillingCreditNote", "DirectSaleReturn", "BillingDebitNote", "PurchaseBill", "VendorPayment", "StockLedger"]);
      ...
      <div className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-2xl border border-border bg-background p-5 shadow-xl">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-xs text-muted-foreground">{preview.source.model === "PurchaseBill" ? PURCHASE_BILL_PREVIEW_COPY : preview.safety_text}</p>
            <p className="mt-1 text-xs text-muted-foreground">Source model: {preview.source.model}. Posting does not mutate source financial fields.</p>
          </div>
