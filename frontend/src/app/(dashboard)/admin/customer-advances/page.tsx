      ...
      <div className="space-y-6">
        ...
        <ERPSectionShell title="Advance register" description="Select a source record to preview or execute posting.">
          <div className="overflow-x-auto rounded-xl border">
            <table className="min-w-full text-sm"><thead className="bg-muted/40 text-left"><tr><th className="px-3 py-2">ID</th><th className="px-3 py-2">Customer</th><th className="px-3 py-2">Type</th><th className="px-3 py-2">Status</th><th className="px-3 py-2 text-right">Amount</th><th className="px-3 py-2">Reference</th></tr></thead><tbody>{rows.map((row) => <tr key={String(row.id)} onClick={() => { setSelectedId(Number(row.id)); setPreview(null); }} className={`cursor-pointer border-t ${Number(row.id) === selectedId ? "bg-muted/60" : "hover:bg-muted/40"}`}><td className="px-3 py-2">#{text(row.id)}</td><td className="px-3 py-2">{text(row.customer_id)}</td><td className="px-3 py-2">{text(row.transaction_type)}</td><td className="px-3 py-2">{text(row.status)}</td><td className="px-3 py-2 text-right">{formatRupee(row.amount)}</td><td className="px-3 py-2">{text(row.reference_no)}</td></tr>)}</tbody></table>
          </div>
        ...
        {preview ? <ERPSectionShell title="Posting preview" description={preview.duplicate_posting_protection}><div className="rounded-xl border bg-muted/30 px-3 py-2 text-sm">{preview.event_type} · {formatRupee(preview.amount)} · {preview.status}</div><div className="mt-3 grid gap-3 md:grid-cols-4"><div className="rounded-xl border px-3 py-2 text-sm"><div className="text-xs font-semibold uppercase text-muted-foreground">Posting date</div><div className="mt-1 font-semibold">{text(preview.posting_date)}</div></div><div className="rounded-xl border px-3 py-2 text-sm"><div className="text-xs font-semibold uppercase text-muted-foreground">Financial year</div><div className="mt-1 font-semibold">{text(preview.financial_year_code)}</div></div><div className="rounded-xl border px-3 py-2 text-sm"><div className="text-xs font-semibold uppercase text-muted-foreground">Period</div><div className="mt-1 font-semibold">{text(preview.accounting_period_code)}</div></div><div className="rounded-xl border px-3 py-2 text-sm"><div className="text-xs font-semibold uppercase text-muted-foreground">Period status</div><div className="mt-1 font-semibold">{text(preview.accounting_period_status)}</div></div></div>{preview.blocked_reason ? <div className="mt-3 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-950">{preview.blocked_reason}</div> : null}<div className="mt-3 overflow-x-auto rounded-xl border"><table className="min-w-full text-sm"><thead className="bg-muted/40 text-left"><tr><th className="px-3 py-2">Account</th><th className="px-3 py-2">Description</th><th className="px-3 py-2 text-right">Debit</th><th className="px-3 py-2 text-right">Credit</th></tr></thead><tbody>{preview.lines.map((line, index) => <tr key={`${line.account.id}-${index}`} className="border-t"><td className="px-3 py-2">{line.account.code} - {line.account.name}</td><td className="px-3 py-2">{line.description}</td><td className="px-3 py-2 text-right">{formatRupee(line.debit)}</td><td className="px-3 py-2 text-right">{formatRupee(line.credit)}</td></tr>)}</tbody></table></div></ERPSectionShell> : null}
      </div>
    </ERPPageShell>
  );
}
