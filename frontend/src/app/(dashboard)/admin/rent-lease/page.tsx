  ...
  return (
    <article className="flex min-h-[13rem] flex-col rounded-[1.4rem] border border-border bg-card p-4 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
      <div className="flex items-start justify-between gap-3">
        ...
        <span className={`rounded-full border px-2.5 py-1 text-[11px] font-semibold ${STATUS_CLASS[card.status]}`}>{card.status}</span>
      </div>
      <div className="mt-4 flex-1 space-y-2"><h2 className="text-base font-semibold text-foreground">{card.title}</h2><p className="text-sm leading-6 text-muted-foreground">{card.purpose}</p></div>
      <div className="mt-4 border-t border-border pt-3"><Link href={card.href} className="inline-flex h-10 w-full items-center justify-center rounded-xl border border-border bg-background px-3 text-sm font-semibold text-foreground transition hover:bg-muted">Open</Link></div>
    </article>
  );
}
...
export default function AdminRentLeaseCockpitPage() {
  ...
  async function loadSummary() {
    setLoading(true);
    try {
      setSummary(await getRentLeaseAccountingSummary());
      setError(null);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Failed to load rent/lease accounting summary.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <ERPPageShell
      ...
      stats={[{ label: "Demand records", value: valueOf(summary?.demand_records), tone: "info" }, { label: "Accounting bridge", value: bridgeStatus, tone: bridgeReady ? "success" : "warning" }, { label: "Lucky IDs", value: "Not used", tone: "success" }, { label: "Draws", value: "Not used", tone: "success" }]}
      statusBadge={{ label: bridgeReady ? "Bridge ready" : "Needs mapping", tone: bridgeReady ? "success" : "warning" }}
    >
      <div className="space-y-6">
        ...
        {summary ? <div className={`rounded-[1.25rem] border px-4 py-3 text-sm ${bridgeReady ? "border-emerald-200 bg-emerald-50 text-emerald-950" : "border-amber-200 bg-amber-50 text-amber-950"}`}>
          <div className="flex items-center gap-2 font-semibold"><ShieldCheck className="h-4 w-4" aria-hidden="true" />Rent/lease accounting bridge: {bridgeStatus}</div>
          <p className="mt-1">Source collection is enabled. Posting remains explicit, idempotent, and controlled by backend preview/execute endpoints.</p>
        ...
        <ERPSectionShell title="Rent / lease workflows" description="Detailed child routes live here instead of the admin sidebar. Every card links to an existing route.">
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">{workflowCards.map((card) => <CockpitWorkflowCard key={card.title} card={card} />)}</div>
        </ERPSectionShell>
      </div>
    </ERPPageShell>
  );
