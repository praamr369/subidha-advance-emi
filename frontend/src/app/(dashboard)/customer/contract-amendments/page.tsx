...
import { amendmentContractTypeLabel, amendmentTypeLabel, listCustomerAmendments, type AmendmentRecord } from "@/services/amendments";
...
export default function CustomerContractAmendmentsPage() {
  ...
  return (
    ...
    >
      <div className="space-y-5">
        ...
        <MetricStrip items={[{ label: "Total", value: String(rows.length) }, { label: "Pending", value: String(rows.filter((r) => ["REQUESTED", "UNDER_REVIEW"].includes(r.status)).length) }, { label: "Approved", value: String(rows.filter((r) => r.status === "APPROVED").length) }, { label: "Rejected", value: String(rows.filter((r) => r.status === "REJECTED").length) }]} />
        {loading ? <ERPLoadingState label="Loading amendment requests..." /> : null}
        {!loading && error ? <ERPErrorState title="Unable to load amendment requests" description={error} onRetry={() => void load()} /> : null}
        {!loading && !error && rows.length === 0 ? <ERPEmptyState title="No amendment requests" description="No amendment request is recorded for your account yet." /> : null}
        {!loading && !error && rows.length > 0 ? (
          <DetailPanel title="Request register" description="Read-only register of your submitted amendment requests.">
            <div className="grid gap-3">
              {rows.map((row) => (
                <Link key={row.id} href={`/customer/contract-amendments/${row.id}`} className="rounded-2xl border border-border bg-card p-4 transition hover:border-primary/50">
                  <div className="flex flex-wrap justify-between gap-3">
                    <div>
                      <div className="font-semibold">{row.amendment_no || `AMD-${row.id}`}</div>
                      <div className="mt-1 text-xs text-muted-foreground">{amendmentContractTypeLabel(row.contract_type)} · {sourceLabel(row)}</div>
                    </div>
