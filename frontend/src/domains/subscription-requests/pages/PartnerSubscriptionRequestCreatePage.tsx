...
export default function PartnerSubscriptionRequestCreatePage({
    ...
    async function init() {
      ...
      try {
        const [opts, custs] = await Promise.all([
          getSubscriptionRequestOptions("partner"),
  ...
  useEffect(() => {
    async function refreshBatch() {
      if (!batchId) { setLuckyNumber(""); return; }
      try {
        const payload = await getSubscriptionRequestOptions("partner", { batchId });
        setOptions(payload);
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      const res = await createPartnerSubscriptionRequest(payload as Parameters<typeof createPartnerSubscriptionRequest>[0]);
      setSuccess(res);
    } catch (err: unknown) {
      ...
      <div className="p-4">
        <div className="rounded-2xl border border-border bg-card p-6 text-center shadow-sm">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Request #{success.request.id} is pending admin approval.
          </p>
          {success.request.product_name ? (
            <div className="mt-4 rounded-xl bg-muted/50 p-3 text-sm">
              <span className="font-semibold">{success.request.product_name}</span>
              {success.request.batch_code ? ` · ${success.request.batch_code}` : ""}
            </div>
          ) : null}
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-5 px-3 py-4 sm:px-4 pb-8">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          <h1 className="text-lg font-bold text-foreground">New Subscription Request</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="rounded-2xl border border-border bg-card p-4 shadow-sm space-y-3">
            ...
            {[
              ...
              { label: "Lucky Number", value: luckyNumber ? `#${luckyNumber.padStart(2, "0")}` : "—" },
              { label: "Monthly EMI", value: derivedMonthly ? money(derivedMonthly) : "—" },
            ].map(({ label, value }) => (
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-muted-foreground">Notes for Admin (Optional)</label>
            <textarea
              value={notes}
      ...
      {step < 3 ? (
        <div className="flex gap-3">
          ...
          >
            {step === 2 ? "Review" : "Next"} <ChevronRight className="size-4" />
          </button>
        </div>
      ) : null}
    </div>
