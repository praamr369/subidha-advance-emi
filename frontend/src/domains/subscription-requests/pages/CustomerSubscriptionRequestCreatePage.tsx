...
export default function CustomerSubscriptionRequestCreatePage({
  ...
  useEffect(() => {
    async function init() {
      setLoading(true);
      try {
        const opts = await getSubscriptionRequestOptions("customer");
        setOptions(opts);
  ...
  useEffect(() => {
    async function refreshBatch() {
      ...
      try {
        const payload = await getSubscriptionRequestOptions("customer", { batchId });
        setOptions(payload);
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setSubmitting(true);
    try {
      const res = await createCustomerSubscriptionRequest({
      ...
      <div className="p-4">
        <div className="rounded-2xl border border-border bg-card p-6 text-center shadow-sm">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Your request #{success.request.id} is pending admin review. We&apos;ll notify you when it&apos;s approved.
          </p>
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-5 px-3 py-4 sm:px-4 pb-8">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          <h1 className="text-lg font-bold text-foreground">New Plan Request</h1>
      ...
      {step < 2 ? (
        <div className="flex gap-3">
          ...
          >
            {step === 1 ? "Review" : "Next"} <ChevronRight className="size-4" />
          </button>
        </div>
      ) : null}
    </div>
