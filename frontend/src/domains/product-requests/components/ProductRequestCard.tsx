...
import ProductRequestActionsBar from "@/domains/product-requests/components/ProductRequestActionsBar";
...
function customerLabel(request: ProductRequestRecord): string {
  return (
      ...
      <div className="min-w-0 space-y-4">
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <div className="rounded-2xl border border-slate-200/80 bg-white/80 px-4 py-3">
            ...
            <div className="mt-2 text-sm font-medium text-slate-900">
              {text(request.approved_subscription_number || request.approved_direct_sale_number, request.status)}
            </div>
          </div>
        </div>
...
export default function ProductRequestCard({
  ...
  function handleStatusChange(updated: ProductRequestRecord) {
  ...
  const content = (
    <>
      <CardBody request={currentRequest} showRequester={showRequester} />
      {showActions && (
        <>
          <div className="mt-4 border-t border-slate-200/80 pt-4">
            <ProductRequestActionsBar
  ...
  if (href) {
    return (
      <Link href={href || `/admin/requests/product-requests/${currentRequest.id}`} className="group block h-full">
