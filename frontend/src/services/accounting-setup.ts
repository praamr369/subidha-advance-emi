...
const ACCOUNT_CODES: Record<string, string[]> = {
  ...
  RENTAL_ASSET_IN_SERVICE: ["INV-1220"],
...
const PROFILE_ROWS = [
  ...
  { key: "rental_asset_handover", label: "Rental Asset Handover (On Hire)", debit: ["RENTAL_ASSET_IN_SERVICE"], credit: ["INVENTORY_ASSET"], implemented: true, operatorNote: "Rent/lease handover is not a sale: the goods stay your asset and are only reclassified out of inventory while they are with the customer. No COGS is posted." },
  { key: "rental_asset_return", label: "Rental Asset Return (Back In Stock)", debit: ["INVENTORY_ASSET"], credit: ["RENTAL_ASSET_IN_SERVICE"], implemented: true, operatorNote: "Reverses the handover reclassification when the unit comes back and can be re-hired." },
