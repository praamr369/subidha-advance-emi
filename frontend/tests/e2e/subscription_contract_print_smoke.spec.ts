...
async function expectSubscriptionScreenNavigationHiddenDuringPrint(page: Parameters<typeof test>[0]["page"]) {
  ...
  await expect(page.getByText("Document generated from existing subscription API payload only")).toBeHidden();
...
test("subscription contract print route renders branded Lucky Plan agreement", async ({ page }) => {
  ...
  await expect(page.getByText("Document generated from existing subscription API payload only")).toBeVisible();
  ...
  await expect(page.getByText("Customer Signature").first()).toBeVisible();
  ...
  await expectSubscriptionScreenNavigationHiddenDuringPrint(page);
});
