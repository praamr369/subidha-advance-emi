...
async function mockReconciliationReportApis(page: Parameters<typeof test>[0]["page"]) {
  await page.route("**/api/v1/admin/reconciliation/**", async (route) => {
    ...
    // Check most specific path FIRST to avoid the list handler swallowing detail requests.
    // Omit trailing slash — some clients omit it even when the Django endpoint expects one.
    if (url.includes("/reconciliation/runs/301")) {
      await route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify(runFixture) });
    } else if (url.includes("/reconciliation/modules/")) {
      await route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ run: runFixture, results: [{ module: "PAYMENTS", open_count: 3, high_risk_count: 1 }] }) });
    } else if (url.includes("/reconciliation/items/")) {
      await route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ count: 1, next: null, previous: null, results: [itemFixture] }) });
    } else if (url.includes("/reconciliation/runs/")) {
      await route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ count: 1, next: null, previous: null, results: [runFixture] }) });
    } else {
...
test("reconciliation report print route renders branded unreconciled report", async ({ page }) => {
  ...
  await expect(page.getByText("RECONCILIATION REPORT").first()).toBeVisible();
  ...
  await expect(page.getByText("Matched Amount", { exact: true })).toBeVisible();
  ...
  await expect(page.getByText("Prepared By Signature").first()).toBeVisible();
  await expect(page.getByText("Reviewer / Admin Signature").first()).toBeVisible();
