...
async function mockPurchaseVendorDocumentApis(page: Parameters<typeof test>[0]["page"]) {
  await page.route("**/inventory/vendor-bills/301/", async (route) => {
    await route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify(vendorBillFixture) });
  });
  await page.route("**/inventory/vendor-bills/**", async (route) => {
    await route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify({ count: 1, next: null, previous: null, results: [vendorBillFixture] }),
    });
  });
  await page.route("**/inventory/vendor-payments/401/", async (route) => {
    await route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify(vendorPaymentFixture) });
  });
  await page.route("**/inventory/vendor-payments/**", async (route) => {
    await route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify({ count: 1, next: null, previous: null, results: [vendorPaymentFixture] }),
    });
  });
  await page.route("**/admin/vendors/701/outstanding/", async (route) => {
    await route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify(vendorOutstandingFixture) });
  });
  await page.route("**/admin/vendors/701/", async (route) => {
    await route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify(vendorFixture) });
  });
  await page.route("**/admin/vendors/**", async (route) => {
...
test("purchase bill print route renders branded vendor bill with unsafe status guard", async ({ page }) => {
  ...
  await expect(page.getByText("Vendor payable remains outstanding")).toBeVisible();
  await expect(page.getByText("Vendor Acknowledgement Signature")).toBeVisible();
