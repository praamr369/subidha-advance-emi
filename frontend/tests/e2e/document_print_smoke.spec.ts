...
async function expectPrintToolbarHiddenDuringPrint(page: Parameters<typeof test>[0]["page"]) {
...
test("direct-sale invoice print route renders branded financial document", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "NON-GST INVOICE" })).toBeVisible();
  await expect(page.getByText("DSI-2026-00001").first()).toBeVisible();
  await expect(page.getByText("Print Smoke Customer")).toBeVisible();
  ...
  await expectPrintToolbarHiddenDuringPrint(page);
});
...
test("receipt print route renders branded receipt and unsafe-status guard surface", async ({ page }) => {
  ...
  await expect(page.getByText("DSI-2026-00001", { exact: true }).first()).toBeVisible();
  await expect(page.getByText("Generated by SUBIDHA CORE")).toBeVisible();
  await expect(page.getByText("Authorized Signature", { exact: true })).toBeVisible();
  await expect(page.getByText("Customer Signature", { exact: true })).toBeVisible();
  await expect(page.getByRole("button", { name: "Print / Save PDF" })).toBeVisible();
...
test("direct-sale workspace exposes branded invoice PDF link", async ({ page }) => {
  ...
  await expect(page.getByText("DSI-2026-00001")).toBeVisible();
...
test("direct-sale delivery challan print route renders branded challan", async ({ page }) => {
  ...
  await expect(page.locator("header").getByText("Subidha Furniture", { exact: true })).toBeVisible();
  await expect(page.getByRole("heading", { name: "DELIVERY CHALLAN" })).toBeVisible();
  await expect(page.getByText("DCH-PRINT-901").first()).toBeVisible();
  await expect(page.getByText("Print Smoke Customer")).toBeVisible();
  await expect(page.getByText("Receiver Smoke", { exact: true }).first()).toBeVisible();
  await expect(page.getByText("DSI-2026-00001", { exact: true }).first()).toBeVisible();
  await expect(page.getByText("Court More")).toBeVisible();
  ...
  await expect(page.getByText("Generated by SUBIDHA CORE")).toBeVisible();
  await expect(page.getByText("Staff Signature", { exact: true })).toBeVisible();
  await expect(page.getByText("Receiver Signature", { exact: true })).toBeVisible();
  await expect(page.getByRole("button", { name: "Print / Save PDF" })).toBeVisible();
